...
- Abracadabra `cook` exchange-rate guard naming is easy to misread: the inspected V3 contracts name the argument `maxRate` while requiring the updated rate to be greater than it in `abracadabra-money/magic-internet-money/contracts/CauldronV3.sol:455-458` and sibling variants. The catalog records this as a stale-price policy caveat, not a naming pattern.
