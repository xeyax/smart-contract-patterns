...
- Yearn V3 vault periphery's `DebtAllocator.sol` gates debt updates by change size, wait time, idle buffer, strategy liquidity, max debt, base fee, and unrealized-loss checks in [`src/debtAllocators/DebtAllocator.sol`](https://github.com/yearn/vault-periphery/blob/bc4eee4051e3319427012e65296110bbdc00488d/src/debtAllocators/DebtAllocator.sol).
...
- Kamino KVault allocates vault assets into whitelisted lending reserves with target allocations, per-reserve caps, an unallocated bucket, stale-reserve and invest-delay checks, and post-CPI balance reconciliation in [`programs/kvault/src/state.rs`](https://github.com/Kamino-Finance/kvault/blob/ac367c3adecf9ae29e384b50c9cf2e7fcc521f94/programs/kvault/src/state.rs), `handlers/handler_update_reserve_allocation.rs`, `operations/vault_operations.rs`, and `operations/vault_checks.rs`.
- Tokemak V2 executes solver-driven flash rebalances only after strategy verification of time gaps, slippage, idle thresholds, destination premium/discount, and predicted swap-cost recovery in [`src/vault/AutopoolETH.sol`](https://github.com/Tokemak/v2-core-pub/blob/de163d5a1edf99281d7d000783b4dc8ade03591e/src/vault/AutopoolETH.sol) and [`src/strategy/AutopoolETHStrategy.sol`](https://github.com/Tokemak/v2-core-pub/blob/de163d5a1edf99281d7d000783b4dc8ade03591e/src/strategy/AutopoolETHStrategy.sol).
- mStable savings contracts rebalance a bounded connector fraction on cadence, validate connector yield, and refresh exchange rates after allocation changes in [`contracts/savings/SavingsContract.sol`](https://github.com/mstable/mStable-contracts/blob/51da0272104d207abcbecb5dd545fec2e6abbfe9/contracts/savings/SavingsContract.sol).
