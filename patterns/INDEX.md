...
| pattern-clarity-trait-cohort-validation.md (clarity) | Validate a caller-supplied cohort of trait-typed Clarity contracts by checking each contract identity against configured roles before using the cohort. | A Clarity entrypoint receives trait-typed token, oracle, or wrapper contracts |
| pattern-consumer-scoped-rate-limiter.md | Apply token-bucket limits per approved consumer or route so one actor cannot exhaust shared capacity for unrelated flows. | Multiple protocol components share a constrained operation such as instant redemption, bridge egress, or privileged … |
...
| pattern-merkle-scoped-sale-escrow-caps.md | Gate token-sale deposits through per-user escrows whose Merkle leaves bind owner, sale registry, and personal cap. | A permissioned sale needs per-user caps without storing the full allowlist on-chain |
...
| pattern-pda-scoped-protocol-authority.md (solana) | Derive Solana protocol authorities and custody accounts from canonical PDA seeds, then verify every account against the stored authority graph. | A Solana program owns token vaults, mints, obligations, or market authority through PDAs |
...
| pattern-solana-account-cohort-validation.md (solana) | Validate every passed Solana account as part of the expected account cohort before trusting its data, authority, or balance. | A Solana instruction receives many accounts from the caller |
...
| pattern-cranked-validator-set-maintenance.md (solana) | Maintain large validator sets through an explicit on-chain phase machine with bounded per-validator cranks and epoch gates. | Validator-set maintenance cannot fit in one transaction or instruction |
...
| pattern-read-only-keeper-trigger-facade.md | Expose keeper work as read-only trigger checks that return whether execution is needed and the exact calldata to run. | Keepers need to discover report, tend, rebalance, auction, or claim work off-chain |
...
| pattern-pairwise-bridge-rate-limits.md | Rate-limit bridge flow per source and destination peer so one route cannot drain or flood every other route. | A bridge token has multiple trusted peers or endpoints |
...
| pattern-relayer-funded-native-drop-accounting.md | Separate bridge token redemption from destination native-token funding so users can receive gas while relayers recover bounded funding and fees. | A bridge transfer may need destination native gas for the recipient |
...
| pattern-route-scoped-dvn-quorum.md | Configure required and optional verifier sets per remote route, then accept a packet only when required verifiers and an optional threshold satisfy the route policy. | A messaging route can choose its own verifier set and confirmation policy |
| pattern-route-scoped-message-library-migration.md | Migrate cross-chain message libraries per application route while accepting the old receive library for a bounded grace window. | Applications can choose send or receive libraries per remote endpoint |
...
| pattern-threshold-delayed-bridge-payout.md | Delay large validator-approved bridge payouts behind a public execution window while allowing smaller payouts to settle immediately. | A bridge already has a validator, oracle, or operator approval path |
...
| pattern-typed-cross-chain-executor-options.md | Bind destination execution gas, value, ordering, and compose requirements into typed options that the route can enforce before fee quoting or delivery. | Destination messages can need different gas or native-value budgets by message type |
...
| pattern-managed-voting-escrow-aggregator.md | Aggregate user voting-escrow locks into managed veNFT positions that coordinate deposits, withdrawals, rewards, and gauge voting as one managed entity. | Voting escrow positions are tokenized as NFTs or transferable lock records |
...
| pattern-consumable-borrow-right-credit-meter.md | Represent borrow capacity as a transferable token that is continuously consumed by outstanding debt over time. | Borrowing should have a separate prepaid or market-priced right instead of an interest rate charged on principal |
...
| pattern-dual-mode-collateral-shares.md | Split supplied collateral into protected non-borrowable shares and borrowable collateral shares with explicit transitions between modes. | Users can supply assets either as passive protected collateral or as collateral that backs debt |
...
| pattern-lazy-credit-amortization-index.md | Convert harvested yield into a global credit index that lazily amortizes each account's signed debt when the account interacts. | Borrower debt should be repaid by strategy yield over time |
...
| pattern-receipt-wrapped-nft-collateral-loan.md | Custody each pledged NFT in the lending protocol while minting a borrower-held receipt NFT that tracks the loan state. | Each collateral item is unique and cannot be merged into a fungible collateral balance |
| pattern-repayment-liveness-fallback-escrow.md | Let borrowers repay and release collateral even when the lender cannot receive the repayment token directly, by escrowing lender proceeds. | Loan repayment sends funds to a lender, seller, or counterparty that may be blocked by token policy |
...
| pattern-sealed-double-auction-term-lending.md | Match fixed-maturity borrowers and lenders through sealed bid and offer lockers, then clear the auction at a uniform rate. | A market needs fixed-term borrowing and lending at auction-discovered rates |
...
| pattern-signed-fixed-maturity-fcash-ledger.md | Track fixed-maturity asset and debt claims as signed balances that settle into cash after maturity. | A protocol needs positive fixed-maturity claims and negative fixed-maturity debt in one ledger |
...
| pattern-solana-instruction-paired-flash-loan.md (solana) | On Solana, validate flash-loan borrow and repay instructions as a matched pair inside the same transaction. | A Solana lending program offers atomic flash loans; The transaction can include both borrow and repay instructions |
| pattern-tick-weighted-transmuter-redemption.md | Redeem synthetic debt deposits into underlying by advancing active ticks and accumulated weights instead of iterating every depositor. | Users deposit synthetic assets into a redemption queue for future underlying |
| pattern-tranched-credit-line-payment-waterfall.md | Route borrower repayments through a credit-line accounting waterfall that allocates senior and junior tranche principal, interest, fees, and losses. | A borrower draws against a credit line funded by senior and junior capital |
...
| pattern-collateralized-option-liquidity-pool.md | Split option-pool liquidity into free and reserved collateral while minting long/short option claims from the same ERC1155-style ledger. | Liquidity providers underwrite options from shared collateral |
...
| pattern-instruction-paired-rebalance-solvency-record.md (solana) | Bracket an external rebalance with start/end instructions that lock the pool, record pre-rebalance value, and reject completion unless solvency is preserved. | A pool must route inventory through external programs to rebalance reserves |
...
| pattern-polynomial-stable-pair-invariant.md | Price stable-pair swaps with a symmetric polynomial invariant such as `x^3y + y^3x` instead of an amplified `D` invariant. | A two-asset pair should trade near parity or a shared redemption value |
...
| pattern-paid-early-oracle-update-liquidation.md | Delay ordinary oracle reads while allowing liquidation callers to pay for fresh rounds and share the resulting opportunity with the protocol. | Oracle extractable value exists around liquidation price updates |
...
| pattern-upgrade-slot-pinned-rate-adapter.md (solana) | Pin an upgradeable upstream program's last upgrade slot and fail valuation until governance accepts the new slot. | A rate adapter reads state from an upgradeable upstream Solana program |
...
| pattern-snapshot-gated-integration-reward-distribution.md (solana) | Distribute rewards only after integration state, debt state, and processed bitmaps are finalized for the same distribution snapshot. | Rewards depend on off-chain or cross-program accounting snapshots |
...
| File | Description | Use When |
|------|-------------|----------|
| pattern-instruction-paired-flash-order-settlement.md (solana) | Settle a routed order across a start/end instruction pair by locking order state, measuring output balance deltas, and validating the paired end instruction before releasing maker inventory. | Makers post on-chain inventory but takers need to route through arbitrary swap legs before final settlement |
| pattern-instruction-scoped-signed-message-orders.md (solana) | Verify signed order messages against the exact instruction, signer, payload fields, expiry, and replay cache before settlement. | Orders are signed off-chain but verified inside an execution instruction |
...
| pattern-extension-gated-transfer-fee-normalization.md (solana) | Support deterministic token-program transfer fees by reading canonical extension state, normalizing included/excluded amounts, and rejecting unsupported extensions. | The token program exposes canonical, inspectable transfer-fee extension state |
...
| File | Description | Use When |
|------|-------------|----------|
| pattern-account-scoped-rebasing-credit-ledger.md | Track rebasing supply through global credits per token while allowing selected accounts to opt into non-rebasing or delegated-yield credit modes. | A token distributes yield by rebasing balances; Some accounts, wrappers, or integrations need non-rebasing balances |
...
| pattern-oversubscribed-pro-rata-sale-escrow.md | Accept capped sale deposits into escrow, allocate tokens pro rata when demand exceeds supply, and refund unused payment plus fee overages. | A token sale may be oversubscribed; Buyers deposit payment before the final allocation ratio is known |
...
| pattern-token2022-transfer-hook-policy-gate.md (solana) | Use Solana Token-2022 transfer hooks and extra account metadata to enforce policy state on every token transfer. | A token needs transfer-time policy checks such as pause, allowlist, or compliance gates |
...
| File | Description | Use When |
|------|-------------|----------|
| pattern-action-scoped-nav-range-accounting.md | Maintain midpoint, minimum, and maximum debt valuations so deposits and withdrawals use conservative NAV views for their action. | Vault assets include external positions whose realizable value is a range |
| pattern-adapter-routed-index-rebalance.md | Rebalance an index token toward manager targets through approved traders and exchange adapters with cooldowns and per-trade limits. | A portfolio manager sets target component units for an index token |
...
| pattern-epoch-rolled-covered-option-vault.md | Roll vault collateral into short option positions on discrete epochs, auction the minted options, and settle round price-per-share after expiry. | Vault collateral is sold as covered calls, puts, or similar short option exposure |
...
| pattern-hysteretic-strategy-debt-allocator.md | Rebalance vault strategy debt only when deviation, elapsed time, idle liquidity, strategy liquidity, and gas conditions justify moving capital. | A vault allocates capital across multiple strategies |
...
| pattern-module-owned-portfolio-position-ledger.md | Store portfolio-token component positions as module-owned default and external units so integrations can mutate positions without bypassing the core ledger. | A tokenized portfolio holds many component assets and integrations |
