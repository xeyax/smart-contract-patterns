...
**Fix:** Bounded batch processing, pagination, or restructure to avoid loops. Lazy time-bucket catchup loops need a max backlog, public batching, or keeper incentives. If users can append entries to another account's array, require a minimum economic size, owner-only append semantics, or pagination so attackers cannot gas-DoS a victim's aggregate views or withdrawals. Relay-style view aggregators over operators, keys, vaults, reward addresses, or validator sets can be acceptable for off-chain indexing, but should not become on-chain dependencies without pagination or hard caps. Loop-limit helpers must bound the actual loop cardinality; checking half of a later full-length loop is a false guard.
