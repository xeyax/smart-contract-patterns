...
```mermaid
graph LR
  FR-001[FR-001 Open session] --> FR-004[FR-004 Process input]
  FR-005[FR-005 Close session] -. implicit .-> FR-001
  FR-008[FR-008 Reuse cached result] -. implicit .-> FR-004
```
...
- **Implicit dependencies not stated**:
  - FR-005 implies FR-001 (close requires prior open) — not stated explicitly.
    [GAP]: FR-005 implies FR-001 but does not state it explicitly
```
