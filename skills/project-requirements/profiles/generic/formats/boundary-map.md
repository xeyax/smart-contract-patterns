...
|------------|------|---------------|-------------|--------------|--------|
| External service API | runtime | C-003 | FR-007 | — | [GAP]: no failure mode |
| User-provided input file | runtime | C-005 | FR-001, FR-002 | R-004 (file unreadable / malformed) | complete |
| Notification provider | runtime | — | FR-010 | — | [GAP]: declared neither, two gaps |

## B. Environment / build-time / packaging constraints

| Constraint | Kind | Declared by C/NFR | Notes |
|------------|------|--------------------|-------|
| Supported runtime version ≥ X.Y | environment | C-001 | Single-version target |
| Allowed runtime dependencies | environment | NFR-007 | Specific list enumerated in NFR-007 |
| Target deployment platform | environment | C-002 | Linux + macOS |
| Build toolchain ≥ Z | environment | — | [GAP]: build toolchain mentioned in Purpose, no C |

## Boundary violations

- FR-NN describes external behavior ("external service returns JSON within 100ms") — should be a Constraint, not an FR.
